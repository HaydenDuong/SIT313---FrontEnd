...
const ArticlesPage = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      const articlesSnapshot = await getDocs(collection(db, 'posts'));
      const articlesList = articlesSnapshot.docs
        .filter(doc => doc.data().postType === 'article')
        .map(doc => ({ ...doc.data(), id: doc.id }));
      
      setPosts(articlesList); // Set all posts
      setFilteredPosts(articlesList); // Initially show all posts in filtered state
    };
